...
module Kenna
  module Toolkit
    class CheckmarxSca < Kenna::Toolkit::BaseTask
      include Kenna::Toolkit::CheckmarxScaHelper
      ...
      def run(opts)
        ...
        projects.each do |project|
          ...
          asset = { "external_id" => project["id"], "application" => project["name"] }

          # Request checkmarx sca api to fetch all scans of each project
          scans = fetch_all_scans_of_project(token, project["id"])
          vulnerabilites = fetch_all_vulns_of_project(token, scans[0]["scanId"])
          vulnerabilites.each do |vuln|
            finding = {
              "scanner_identifier" => vuln["packageId"],
            ...
            vuln_def["cwe_identifiers"] = vuln["cwe"] if vuln["cwe"].present? && vuln["cwe"] != "NVD-CWE-Other"
